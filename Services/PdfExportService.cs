...
// AÑADIMOS ESTOS ALIAS PARA RESOLVER LA AMBIGÜEDAD
...
public class PdfExportService
{
    ...
    private void BuildHeader(QuestPDFContainer container, DteViewModel vm)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().Text(vm.Dte.Identificacion.FecEmi).AlignRight();
                col.Item().Text(vm.Dte.Identificacion.HorEmi).AlignRight();
            });
        });
    }

    // Y USAMOS EL ALIAS AQUÍ
